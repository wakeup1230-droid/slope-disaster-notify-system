...
def query_stake_number():
    try:
        if entry_latitude.get() and entry_longitude.get():
            ...
            messagebox.showerror("輸入錯誤", "請輸入有效的經緯度或TWD97二度分帶坐標")
            return
    except ValueError:
...
# 說明文字
label_note = tk.Label(frame_output, text="※本程式相關省道里程樁號查找資料庫來源係以\"政府資料開放平臺\"為主，該資料庫為公路局32個養護工務段同仁於現地里程牌旁，利用平板電腦GPS量測坐標，誤差約25公尺；建議仍需以現場里程位置或搭配Google Earth街景進行對位。",
                      font=("Arial", 10), justify="left", wraplength=400)
label_note.pack(pady=10)

# 圖片與版本號
frame_bottom = tk.Frame(frame_output)
...
if os.path.exists(logo_path):
    ...
    label_logo.grid(row=0, column=0, padx=5)

label_version = tk.Label(frame_bottom, text="黃韋凱 製作 Ver3.0", font=("Arial", 10))
label_version.grid(row=0, column=1, padx=5)
