...
def get_nearest_stake_number(latitude=None, longitude=None, twd97_x=None, twd97_y=None):
    if latitude is not None and longitude is not None:
        ...
        def distance(row):
            return ((row['坐標-X-TWD97'] - twd97_x) ** 2 + (row['坐標-Y-TWD97'] - twd97_y) ** 2) ** 0.5
    else:
        return None, None
...
# 新增經緯度和TWD97二度分帶坐標輸入框和按鈕
label_latitude = tk.Label(frame_input, text="請輸入緯度 (WGS84):", font=("Arial", 10))
label_latitude.grid(row=4, column=0, sticky='w')
entry_latitude = tk.Entry(frame_input)
entry_latitude.grid(row=5, column=0, pady=5)
...
label_logo.pack(side=tk.LEFT, padx=10, pady=10)

label_version = tk.Label(frame_bottom, text="黃韋凱 製作 Ver2.0", font=("Arial", 10))
label_version.pack(side=tk.LEFT, padx=10, pady=10)

frame_bottom.pack(side=tk.LEFT, anchor='sw')
